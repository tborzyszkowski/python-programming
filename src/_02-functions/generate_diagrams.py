...
def generate_png(puml_path: pathlib.Path, pl: plantuml.PlantUML) -> pathlib.Path:
    ...
    # Biblioteka plantuml-interface wymaga URL serwera, ale tutaj
    # używamy bezpośrednio processes z pakietu plantuml
    try:
        # Metoda zwraca bajty z obrazkiem
        png_data = pl.processes(source)
    except Exception as e:
        # Jeśli processes failuje (np. zły URL), próbujmy inaczej lub rzućmy błąd
        # Ale biblioteka `plantuml` (pypi plantuml) ma metodę processes.
        # Sprawdzmy czy to jest na pewno `plantuml` a nie `plantuml-markdown`.
        # Zakładam że użytkownik zainstalował `plantuml` jak w requirements.txt
        raise RuntimeError(f"Błąd generowania PNG: {e}")
...
def main() -> None:
    ...
    try:
        ...
        # Fallback jeśli biblioteka ma inne API (np. plantuml-markdown)
        print("Błąd: Upewnij się, że masz zainstalowane `plantuml` (pip install plantuml).")
        sys.exit(1)

    errors: list[tuple[pathlib.Path, str]] = []

    # Szukamy klasy PlantUML w module plantuml
    # W bibliotece 'plantuml' na PyPI klasa nazywa się PlantUML

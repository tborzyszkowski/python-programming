...
def generate_png(puml_path: pathlib.Path, pl: plantuml.PlantUML) -> pathlib.Path:
    ...
    try:
        ...
        raise RuntimeError(f"Blad generowania PNG: {e}")
    if not png_data:
        raise RuntimeError(f"Serwer zwrocil puste dane dla: {puml_path.name}")
...
def main() -> None:
    ...
    try:
        ...
        print("Blad: Upewnij sie, ze masz zainstalowane `plantuml` (pip install plantuml).")
        sys.exit(1)
    ...
    print(f"Wynik: {ok}/{len(puml_files)} wygenerowanych pomyslnie.")
    if errors:
        print("\nNieudane:")
        for p, msg in errors:
